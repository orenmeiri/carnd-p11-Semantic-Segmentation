...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(logits=nn_last_layer, labels=correct_label_flat)#, name='xentropy'))
    cost = tf.reduce_mean(cross_entropy_loss)
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)

    return logits_flat, optimizer, cost
tests.test_optimize(optimize)


def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
             correct_label, keep_prob, learning_rate, runs_dir="", data_dir="", image_shape="", logits="", saver = ""):
    """
    ...
    for epoch in range(epochs):
        ...
        for images, gt_images in get_batches_fn(batch_size):
            logins, cost = sess.run([train_op, cross_entropy_loss], feed_dict={input_image : images, correct_label: gt_images, keep_prob:0.5})
            total_cost += cost
            total_cost_count += 1
        print("finished epoch {}".format(epoch))
        print("cost: {}".format(total_cost / total_cost_count))
        if epoch != 0 and epoch % 10 == 0:
            helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image,
                                          subdir='data_road/testing_some')
        # if epoch != 0 and epoch % 5 == 0:
        #     saver.save(sess, "semantic-segmentation", global_step = epoch)
    print("training DONE")
...
def run():
    ...
    with tf.Session() as sess:
        ...
        correct_label = tf.placeholder(tf.int32, shape=(None, image_shape[0], image_shape[1], 2))
        ...
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image, correct_label, keep_prob, learning_rate, runs_dir, data_dir, image_shape, logits, saver)
        saver.save(sess, "semantic-segmentation_0.5dropout", global_step = epochs)
